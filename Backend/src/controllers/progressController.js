...
const seedProgressForStudent = async (req, res) => {
  ...
  try {
          ...
          return {
            ...
            tasks: st.tasks.map(t => {
              ...
              return {
                ...
                ordering: t.ordering,
                status: 'Locked',
                subtasks: t.subtasks.map(s => ({ _id: new mongoose.Types.ObjectId(), title: s.title, description: s.description, ordering: s.ordering, status: 'Locked' }))
              };
            })
          };
        })
    ...
    const inst = await ProgressInstance.create({ studentId, templateId, milestones });
...
const getStudentProgress = async (req, res) => {
  try {
    const inst = await ProgressInstance.findOne({ studentId: req.params.studentId });
    if (!inst) return res.status(404).json({ message: 'No progress found' });
    return res.json(inst);
  } catch (err) { console.error(err); return res.status(500).json({ message: 'Server error' }); }
};
...
const updateNodeStatus = async (req, res) => {
  ...
  try {
    ...
    inst.audit.push({ when: new Date(), by: req.user._id, action: 'update_status', nodeId, oldStatus: found.oldStatus, newStatus: status });
    await inst.save();
    return res.json(inst);
  } catch (err) { console.error(err); return res.status(500).json({ message: 'Server error' }); }
};
...
const overrideNodeStatus = async (req, res) => {
  const { nodeId } = req.params; const { status } = req.body;
  try {
    const inst = await ProgressInstance.findOne({ studentId: req.params.studentId });
    if (!inst) return res.status(404).json({ message: 'No progress found' });
    const found = setNodeStatusById(inst.milestones, nodeId, status);
    if (!found) return res.status(404).json({ message: 'Node not found' });
    propagateStatus(inst.milestones);
    inst.audit.push({ when: new Date(), by: req.user._id, action: 'override_status', nodeId, oldStatus: found.oldStatus, newStatus: status });
    await inst.save();
    return res.json(inst);
  } catch (err) { console.error(err); return res.status(500).json({ message: 'Server error' }); }
};

module.exports = { seedProgressForStudent, getMyProgress, getStudentProgress, updateNodeStatus, overrideNodeStatus };
