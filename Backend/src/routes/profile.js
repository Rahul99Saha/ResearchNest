...
router.get('/my-profile', auth, async (req, res) => {
  try {
    const user = await User.findById(req.user.id)
      .select('-password')
      .populate('researchProfile.supervisor', 'personalInfo firstName lastName email');
...
router.get('/:userId', auth, async (req, res) => {
  try {
    const requestedUser = await User.findById(req.params.userId)
      ...
      .populate('researchProfile.supervisor', 'personalInfo firstName lastName');
    ...
    if (req.user.personalInfo.department !== requestedUser.personalInfo.department && 
        req.user.role !== 'admin') {
...
router.put('/academic-info', auth, requireRole(['student']), async (req, res) => {
  try {
    const { program, specialization, currentSemester, expectedGraduation } = req.body;
...
router.put('/faculty-info', auth, requireRole(['faculty', 'admin']), async (req, res) => {
  try {
...
router.put('/research-info', auth, async (req, res) => {
  try {
    const { researchTopic, researchArea, researchProgress, publications } = req.body;
...
router.get('/faculty/supervised-students', auth, requireRole(['faculty', 'admin']), async (req, res) => {
  try {
...
router.put('/:studentId/supervisor', auth, requireRole(['admin', 'faculty']), async (req, res) => {
  try {
